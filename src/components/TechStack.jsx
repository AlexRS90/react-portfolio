...
const TechStack = () => {
  ...
  return (
    <div>
      ...
      >
        Skills
        <svg xmlns="http://www.w3.org/2000/svg" className={`h-6 w-6 ${isOpenSkills ? 'rotate-90' : 'rotate-0'}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2">
          <path strokeLinecap="round" strokeLinejoin="round" d="M9 5l7 7-7 7" />
        </svg>
      </button>
      <ul className={`${isOpenSkills ? 'block' : 'hidden'}`}>
        {
          skills.map((skill) => (
            <li
              className="w-9/12 bg-background m-2 p-8"
